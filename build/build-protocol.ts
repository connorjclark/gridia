// yarn ts-node build/build-protocol.ts
...
function save(code: string, name: string) {
  fs.writeFileSync(`./src/protocol/gen/${name}`, code);
}

function collect({node, commandOrEvent}: { node: ts.Node; commandOrEvent: 'command' | 'event' }) {
  ...
  const firstParams = isServerInterface ?
    ['Server', 'ClientConnection'] :
    ['Client'];
  const protocolInterface = createProtocolInterface(
    qualifier, 'I' + node.name.escapedText, firstParams, messageDeclarations);
...
function getMessageDeclarations(
  {node, qualifier, commandOrEvent}: { node: ts.Node; qualifier: ts.Node; commandOrEvent: 'command' | 'event' }
): MessageDeclaration[] {
...
function createMessageType(
  {qualifier, node, commandOrEvent}: { qualifier: ts.Node; node: ts.Node; commandOrEvent: 'command' | 'event' }
) {
  /*
 ...
 * @param {ts.Node} name
 * @param {string[]} paramTypeNames
 * @param {MessageDeclaration[]} messageDeclarations
 */
function createProtocolInterface(qualifier, name, paramTypeNames, messageDeclarations) {
  const members = messageDeclarations.map((messageDeclaration) => {
    const firstParams = paramTypeNames.map((typeName) => {
      return ts.createParameter(
        undefined,
        undefined,
        undefined,
    ...
    const params = [...firstParams, messageParam];
